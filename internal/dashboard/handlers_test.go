...
func newTestDashboard(t *testing.T) (http.Handler, *allow.List) {
	...
	dash := dashboard.New(cfg, evLog, logger, al, nil, nil, nil, nil, nil, nil)
	r := chi.NewRouter()
	dash.Mount(r)
	return r, al
}
...
func TestHandleAllowList_NilAllowList(t *testing.T) {
	...
	evLog := eventlog.New(50)
	logger := zerolog.Nop()
	dash := dashboard.New(cfg, evLog, logger, nil, nil, nil, nil, nil, nil, nil)
	r := chi.NewRouter()
	dash.Mount(r)
...
func TestHandleAllow_UnauthenticatedRejected(t *testing.T) {
	...
	al, err := allow.New("")
	require.NoError(t, err)
	dash := dashboard.New(cfg, eventlog.New(10), zerolog.Nop(), al, nil, nil, nil, nil, nil, nil)
	r := chi.NewRouter()
	dash.Mount(r)
...
func TestHandleEvents_Since(t *testing.T) {
	...
	cfg := config.DashboardConfig{Enabled: true, Path: "/dashboard",
		...
		Secret: "aabbccddeeff00112233445566778899"}
	dash := dashboard.New(cfg, evLog, zerolog.Nop(), nil, nil, nil, nil, nil, nil, nil)
	r := chi.NewRouter()
	dash.Mount(r)
