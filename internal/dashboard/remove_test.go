...
func newDashWithBothLists(t *testing.T) (http.Handler, *allow.List, *block.List, *eventlog.Log) {
	...
	dash := dashboard.New(cfg, evLog, zerolog.Nop(), al, bl, nil, nil, nil, nil, nil, "", nil)
	r := chi.NewRouter()
	dash.Mount(r)
...
func TestHandleAllowRemove_NilAllowList(t *testing.T) {
	...
	dash := dashboard.New(cfg, eventlog.New(10), zerolog.Nop(), nil, nil, nil, nil, nil, nil, nil, "", nil)
	r := chi.NewRouter()
	dash.Mount(r)
